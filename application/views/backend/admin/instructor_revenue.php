                                    ...
                                    <td style="text-align: center;">
                                        <?php if ($payment['instructor_payment_status'] == 0): ?>
                                            <?php if ($paypal_keys[0]['production_client_id'] != ""): ?>
                                                <form action="<?php echo site_url('payment/llojipageses/paypal_checkout_for_instructor_revenue'); ?>" method="post">
                                            ...
                                            <?php if ($stripe_keys[0]['public_live_key'] != "" && $stripe_keys[0]['secret_live_key']): ?>
                                                <form action="<?php echo site_url('payment/llojipageses/stripe_checkout_for_instructor_revenue'); ?>" method="post">
                                                    <input type="hidden" name="amount_to_pay"   value="<?php echo $payment['instructor_revenue']; ?>">
                                                    <input type="hidden" name="payment_id"      value="<?php echo $payment['id']; ?>">
                                                    <input type="hidden" name="instructor_name" value="<?php echo $user_data['first_name'].' '.$user_data['last_name']; ?>">
                                                    <input type="hidden" name="course_title"    value="<?php echo $course_data['title']; ?>">
                                            ...
                                            <?php endif; ?>
                                        <?php else: ?>
                                            <a href="<?php echo site_url('payment/llojipageses/invoice/'.$payment['id']); ?>" class="btn btn-outline-primary btn-rounded btn-sm"><i class="mdi mdi-printer-settings"></i></a>
                                        <?php endif; ?>
                                    </td>
                                </tr>
                            <?php endforeach; ?>
